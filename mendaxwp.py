...
import urlparse
from mendaxcolors import Style,Fore,Back,init 
init()
if os.name == 'nt':
	os.system('cls')
else:
...
def mendaxwp():
	print("  __  __ _____ _   _ ____    _    __  __ ")
	print(" |  \/  | ____| \ | |  _ \  / \   \ \/ / ")
	print(" | |\/| |  _| |  \| | | | |/ _ \   \  /  ")
	print(" | |  | | |___| |\  | |_| / ___ \  /  \  ")
	print(" |_|  |_|_____|_| \_|____/_/   \_\/_/\_\ Wordpress Pentest [v2.1] ")
	print("") 
	print(" "+boldred+"MENDAXWP - Mendax Yazilim kullanicilari icin hazirlanmis WP Pentest Araci"+reset)
	print(" "+boldred+"YAZAR: EGEMEN KAYA - ( https://mendaxyazilim.com )"+reset)
        print(" "+green+"KOPYALANMASI VE GELISTIRILMESI SERBESTTIR!"+reset)
        print("")
	def menu():
		global filedetector,wpenumerator,subdomainscan,portscan,wpscan,xssscanner,wpbackupscan,sqliscanner
		print(" -- "+green+"Menu"+reset+" -- \n \n  1. 	"+boldwhite+"WordPress Kullanici Adi Bulucu"+reset+"   \n  2. 	"+boldwhite+"Hassas Dosya Bulucu"+reset+"        \n  3. 	"+boldwhite+"Sub-Domain Tarayici"+reset+"\n  4. 	"+boldwhite+"Port Tarayici"+reset+"        \n  5. 	"+boldwhite+"Wordpress Tarayici\n"+reset+"  6. 	"+boldwhite+"Cross-Site Scripting [ XSS ] Tarayici\n"+reset+"  7.    "+boldwhite+"Wordpress Yedekleme Bulucu\n"+reset+"  8.    "+boldwhite+"SQL Injection [ SQLI ] Tarayici\n"+reset)
		option = raw_input(" "+boldred+"[+] Bir secenek secin"+reset+"\n    > ")
		if option == "1":
			wpenumerator = "on"
		elif option == "2":
			filedetector = "on"
	...
	def cloudflare():
		...
		if "used CloudFlare to restrict access</title>" in pagesource:
			print("[!] Cloudflare IP'yi Engelledi")
			again()
	def alive():
		try:
			...
			print("[i] "+green+"Site Bulundu!"+reset)
			print("  \n[+] Hedef Bilgisi:\n | URL: "+boldwhite+"%s"+reset+"\n | IP: "+boldwhite+"%s"+reset+"\n  ")%(data,ip)
			print("[+] Cloudflare'in erisimi engelleyip engellemedigini kontrol ediliyor...")
			cloudflare()
			redirectcheck()
		except(IOError):
			print("[!] "+red+"Siteye baglanirken hata olustu! Site kapali olabilir."+reset)
			again()
	def responseheadercheck():
		...
		for i in page.headers:
			if i.lower() in str(headers).lower():
				...
				headersfound.append(structure)
		if cj == 0:
			caution.append("[!]"+red+" X-Frame-Options Basligi Kayip\n"+reset+"[!] "+red+"Sayfa savunmasiz olabilir "+boldred+"Click Jacking\n"+reset+"[!] "+boldred+page.geturl()+reset+"\n[i] ClickJacking Hakkinda: [ "+green+"https://www.owasp.org/index.php/Clickjacking"+reset+" ]")
		print("[+] Ilginc Basliklar Bulundu:")
		for i in headersfound:
			print(" | %s")%(i)
		if len(interesting) != 0:
			print("\n[i] Basliktan Bilgiler:")
	...
	def parameterarrange(payload):
		...
		for n in parameters:
			try:
				print("Parametreler '%s' Kontrol Ediliyor")%n[0]
		...
		for n in parameters:
			...
			try:
				for i in payloads:	
					...
					source = u.read()
					#print ("[+] HTML Context Kontrol Ediliyor...")
			...
			if found == 1:
				print("[!] "+red+"SQL Injection Acigi Bulundu!"+reset)
				print dbfound
				vuln.append("'"+n[0]+"'")
				found = 0
		if len(vuln) != 0:
			print(" | Hassas Parametre/ler:"), 
			for i in vuln:
				print(i),

		else:
			print("[!] Savunmasiz Degil")
	def XSSscan(site):
		print("[+] [ XSS ] Tarayici Basladi...")
		vuln = []
		payloads = {
					'3':'d4rk();"\'\\/}{d4rk',
		...
		for n in parameters:
				...
				for i in payloads:	
					...
					if str(i) == str(1):
						...
						if payloads[i] in source:#htmlcode[0].contents[0]:
							#print("[+] XSS Acigi Bulundu.")
							found = 1
					script = code.findAll('script')
					if str(i) == str(3) or str(i) == str(2):
						#print("[+] JS Context Kontrol ediliyor...")
						if str(i) == str(3):
							#JS Context
			...
			if found == 1:
				...
				found = 0
		if len(vuln) != 0:
			print("[!] "+red+"Hassas Parametre/ler:"+reset), 
			for i in vuln:
				print(boldred+i+reset),
		else:
			print("[!] Savunmasiz Degil")
	def portscanner():
		print("[i] Sozdizimi	:	Fonksyon")
  		print("    23,80,120	:	Spesifik Portlari Tara, ornek, Scans Port 23,80 and 120")
  		print("    23-80	:	Belirli Port Arasini Tara, ornek, Scans Port from 23 to 80")
  		print("    23   	:	Tek Portu Tara, ornek, Scans Port 23")
  		print("    all  	:	20 ile 5000 Arasinda ki Tum Portlari Tara")
  		print(" ")
  		portoption = raw_input("[+] Alan Ya da Port Girin:\n    > ")
  		wasmultiple		 = 		0
  		wasrange		 = 		0
  		wasone			 =		0
  		if ',' in portoption:
		...
		for i in wordlist:
			...
			try:
				...
				if (s):
					...
					if s == '127.0.0.1':
						print("[!] "+red+"Sub-domain savunmasiz "+boldred+"Same-Site Scriptlenmesi! "+reset+"\n[!] Same-Site Scriptlenmesi Hakkinda Bilgi:\n[!] [ "+green+"https://www.acunetix.com/vulnerabilities/web/same-site-scripting"+reset+" ] ")
					print('')
	...
	def enumform(listofIDs,listofnames):
		...
		if lengthofnames < 12:
			lengthofnames = 12
		print "[i] "+green+"Kullanici adi/lari bulundu:"+reset
		print "\t+-"+'-'.center(6, '-')+'-+-'+'-'.center(lengthofnames, '-')+"-+"
		print "\t| "+'ID/ler'.center(6, ' ')+' | '+'Kullanici adi/lari'.center(lengthofnames, ' ')+" |"
		print "\t+-"+'-'.center(6, '-')+'-+-'+'-'.center(lengthofnames, '-')+"-+"
	...
	def wpbackupscanner():
		...
		print("[+] Wordpress Yedekleri Araniyor...")
		print("[?] Not: Gecmek icin CTRL+C Basin\n  ")
		progress = 0
		backup = []
		backupurl = []
		try:
		...
		for ifile,iurl in zip(backup,backupurl):
			print("[!] "+boldred+"Yedekleme Bulundu!\n"+reset+" | "+red+"Dosya Adi: "+boldred+"%s"+reset+"\n | "+red+"URL: "+boldred+"%s\n"+reset)%(ifile,iurl)
	def wpenumeration():
		import time
		...
		url = page.geturl()
		if page.geturl() != host:
			print("[i] Host'a yonlendiriliyor '"+str(url)+"' \n    Yonlendirme Takip Ediliyor...")
			host = page.geturl()
		print("\n[+] Tarama Basladi : "+lightgreen+"%s"+reset) % time.strftime("%c")
		print "[+] Kullanici Adlari Sayiliyor..."
		T = 33
		found = 0
		listofusernames = []	
		listofids = []
		...
		if found == 0:
			print("[+] "+red+"Kullanici Adi Bulunamadi"+reset)
		else:
			enumform(listofids,listofusernames)
		print("[+] Sayim Tamamlandi.")
		print("[+] Tarama Bitti : "+lightgreen+"%s"+reset) % time.strftime("%c")
	def wpscanner():
		print("  \n[+] Wordpress Bulundu")
		wp = 0
		i = 'wp-admin/'
		...
		if "wp-login.php?redirect_to" in page.geturl():
			wp = 1
			print(green+"[i] "+green+"Wordpress Bulundu!"+reset)
			if wpenumeration == "on":
				wpenumeration()
			else:
		...
		if wp == 0:
			...
			if page.getcode() == 200 and "" in page.read():
				print("[!] "+green+"Wordpress Bulundu!"+reset)
				wp = 1
				if wpenumeration == "on":
					wpenumeration()
				else:
					...
					wpenumeration()
		if wp == 0:
			print("[!] "+red+"Wordpress Bulunamadi"+reset)
	def redirectcheck():
		global redirect,host
		redirect = 0
		print("[+] Yonlendirme Kontrolu")
		page = urllib.urlopen(host)
		url = page.geturl()
		if page.geturl() != host:
			option = raw_input("[i] "+boldgrey+"Host yonlendirmeleri "+str(url)+reset+" \n    Bunu Host olarak kabul et? [Y/N]:\n    > ")
			if option.lower() == "y":
				host = page.geturl()
				redirect = 1
		else:
			print("[+] URL yonlendirilemiyor")
	def again():
...
# -- Program Structure Start -- 
	...
	try:
		...
		if filedetector == "on":
			...
			print("[+] MendaxWp Taramayi Baslatti")
			print("[+] Hassas Dosyalar Araniyor...")
			print("[?] Not: Gecmek icin CTRL+C basin\n  ")
			try:
				for i in files:
					if i == "awstats/data/":
						...
						if "<title>Index of /awstats/data</title>" in sourcecode:
							print("[!] awstats tespit edildi!\n[!] URL: %s")%(data)
					else:
						sock(i)
						if page.getcode() == 200:
							print("[!] Dosya Bulundu!\n | Ad: %s\n | URL: %s\n")%(i,data)
			except(KeyboardInterrupt):
				print("\n[+] Dosya Bulunma Gecildi")
		if wpenumerator == "on":
			print("  \n[+] Wordpress Bulundu")
			wp = 0
			i = 'wp-admin/'
